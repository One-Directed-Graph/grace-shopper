...
class Product extends Component {
  ...
  findCat() {
    ...
    if (product.category && categories) {
      dispCat = categories.find((cat) => {
        return cat.name == product.category.name
      })
    }
    return dispCat
  }
  render() {
    const {product, user, order, categories} = this.props

    let similarItems = this.findCat()

    // if (similarItems == undefined && similarItems.products != null) {
    //   console.log(
    //     '<>><><<><><><><>><><><>inside render',
    //     similarItems,
    //     Object.keys(similarItems)
    //   )
    // }
...
const mapState = (state) => {
  const {product, user, order, categories} = state
  return {
    product,
    user,
    order,
